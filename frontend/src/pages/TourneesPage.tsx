...
function normalizeRound(r: Round): NormalizedRound {
  ...
  const statut = statusRaw === 'completed' || statusRaw === 'Terminee' || statusRaw === 'TerminÃ©e' ? 'TerminÃ©e' : 'En cours';
  return {
    nom: r.nom ?? r.name ?? 'â€”',
    depot: r.depot ?? 0,
...
export default function TourneesPage() {
  ...
  const fetchData = useCallback((isAuto = false) => {
    ...
    Promise.all([
      fetch(`/api/connectors/mts1/rounds?date=${selectedDate}`, { headers })
        .then(r => r.ok ? r.json() : Promise.reject(r.status))
        .catch(() => null),
      fetch(`/api/connectors/mts1/anomalies`, { headers })
  ...
  const terminees = tournees.filter(t => t.statut === 'TerminÃ©e').length;

  const formatSecondsAgo = (s: number) => {
    if (s < 10) return 'Ã  l\'instant';
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-orange-100 rounded-xl flex items-center justify-center text-xl">ðŸšš</div>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">TournÃ©es MTS-1</h1>
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {loading ? (
            <span className="text-xs text-gray-400 animate-pulse">Chargementâ€¦</span>
          ) : error ? (
            <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full font-medium">âš  API MTS-1 indisponible</span>
          ) : lastUpdated ? (
            <span className="text-xs text-gray-500 flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse inline-block"></span>
              Live Â· {formatSecondsAgo(secondsAgo)} Â· refresh dans {countdown}s
            </span>
          ) : null}
          <button onClick={() => { fetchData(); setCountdown(REFRESH_INTERVAL); }} disabled={loading} className="flex items-center gap-1 px-3 py-1.5 border border-gray-200 text-gray-600 text-sm rounded-lg hover:bg-gray-50 disabled:opacity-40 transition-colors" title="RafraÃ®chir">ðŸ”„</button>
          <a href="https://console.mts-1.com/rounds" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium rounded-lg transition-colors">Ouvrir MTS-1 â†—</a>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard title="TournÃ©es" value={loading ? 'â€¦' : tournees.length} subtitle="ce jour" icon="ðŸ—ºï¸" color="orange" />
        <KpiCard title="En cours" value={loading ? 'â€¦' : enCours} subtitle={`${terminees} terminÃ©e${terminees > 1 ? 's' : ''}`} icon="ðŸ”„" color="blue" />
        <KpiCard title="Points de livraison" value={loading ? 'â€¦' : totalPdl} subtitle="total du jour" icon="ðŸ“¦" color="green" />
        <KpiCard title="Distance totale" value={loading ? 'â€¦' : `${totalKm.toFixed(0)} km`} subtitle="journÃ©e" icon="ðŸ“" color="purple" />
      </div>
        ...
        <div className="card p-8 text-center text-gray-400">
          {error ? (
            <>
              ...
              <p className="text-sm mt-1">Le token API doit Ãªtre renouvelÃ©. Contactez le support MTS-1.</p>
            </>
          ) : (
            <>
              <p className="text-4xl mb-3">ðŸšš</p>
              <p className="font-medium text-gray-600">Aucune tournÉ½e pour cette date</p>
              <p className="text-sm mt-1">SÃ©lectionnez une autre date ou vÃ©rifiez MTS-1 directement.</p>
            </>
          )}
        </div>
      )}
      ...
      {!loading && tournees.length > 0 && (
        <div className="card overflow-hidden p-0">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-semibold text-gray-800">TournÉ½es du {new Date(selectedDate + 'T12:00:00').toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</h2>
            <div className="flex gap-2 text-xs">
              <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full">{enCours} en cours</span>
              <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full">{terminees} terminÃ©es</span>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-500 text-xs font-medium uppercase tracking-wide">
                  <th className="px-6 py-3 text-left">VÃ©hicule</th>
                  <th className="px-6 py-3 text-center">DÃ©pÃ´ts</th>
                ...
                {tournees.map((t, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-3 text-center">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${t.statut === 'TerminÃ©e' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                        {t.statut === 'TerminÃ©e' ? 'âœ“' : 'âœŽ'} {t.statut}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-3 text-right text-gray-600">{t.m3.toFixed(2)} mÂ³</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-gray-50 font-semibold text-gray-700 text-xs">
                  ...
                  <td className="px-6 py-3 text-right">{tournees.reduce((s,t)=>s+t.m3,0).toFixed(2)} mÂ³</td>
                </tr>
              </tfoot>
            </table>
          </div>
